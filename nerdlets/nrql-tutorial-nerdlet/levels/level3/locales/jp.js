...
const translate = {
  ...
  'Facet cases': {
    ...
    Contents: {
      P1: `
        これまでのレッスンで、 <code>FACET</code> は値を分類したり、
        グループ化して状況を理解する（レスポンスコード別の平均レスポンスタイムを見たりするような場合の）ときに
        便利な手法であることを学んでいただきました。
        <code>FACET</code> を指定する、与えられたアトリビュート値を基にNRDBはデータをブループにまとめます。
        では、レスポンスコードが2xx, 3xx, 4xxのように、いくつかの値を同じグループとしてまとめたいときがあるでしょう。
        そのときは、どうしたらいいでしょうか？
      `,
      P2: `
        <code>FACET CASES</code> を使うと、このような状況でもうまくバケットを分けてクエリの結果が得られるようになります。
        この句は、"<code>WHERE attr OP value</code>" という形式の複数のパラメーターを取ります。
      ...
      H2_1: 'レッスンまとめ',
      P4: `
        <code>FACET CASES</code> は、アトリビュートが持つ複数の値をまとめ、グループ化して扱えるようになります。
