  ...
  'Overriding values': {
    ...
    Contents: {
      ...
      P4: `
        ...
        のような関数を利用できません。このような場合、<5>boolean()</5>または<7>numeric()</7>を使用して
        真偽値や数値に変換します。次の例で利用している<9>average()</9>関数で "httpResponseCode "
        属性を指定しても、値が取得できません。というのも、この属性が文字列だからです。
        しかし、<11>numeric</11>(httpResponseCode)を使って数値として型強制すると、
        <14>average()</14>関数は正常に動作します。
      `,
      P5: `
