...
export default function Wildcards() {
  return (
    <div>
      <p>
        We now know how to use a <code>WHERE</code> clause to filter the results
        in our query. Aside from using standard comparison operators, we can
        also use <code>LIKE</code> and <code>NOT LIKE</code> if we want to
        determine whether an attribute contains or does not contain a specified
        substring. In order to achieve this, we need to use the percent (
        <code>%</code>) symbol as a wildcard anywhere in the string.{' '}
      </p>

      <p>
        In our sample query, we are getting the number of transactions with the
        term "Web" anywhere (beginning, middle or end) in the name.
      </p>
      <SampleQuery
      ...
      <p>
        ...
        word "Web" followed by any text, but then also contains the word "index"
        followed by any number of characters. So, the results will only be
        transactions with "Web" <em>and</em> "index" in the name.
      </p>
      <SampleQuery
      ...
      <p>
        ...
        <code>OR</code> operators, we can simplify our condition by listing the
