...
export default function DiscoverEventsAndAttributes() {
  return (
    <div>
      ...
      <h2>
        <Trans i18nKey="Contents.H1">Discovering Event Types</Trans>
      </h2>
      <p>
        <Trans i18nKey="Contents.P2">
          Let's say you want a list of all event types currently reporting to
          your New Relic account. The <code>SHOW EVENT TYPES</code> syntax
          returns a list of all reported event types in a given period. This is
          one of the rare exceptions where <code>SELECT</code> and{' '}
          <code>FROM</code> are not required in a NRQL query. You could use this
          functionality for things like confirming the existence of custom event
          data, for example.
        </Trans>
      </p>
      <SampleQuery
      ...
      <h2>
        <Trans i18nKey="Contents.H2">Discovering Attributes</Trans>
      </h2>
      <p>
        <Trans i18nKey="Contents.P3">
          ...
          attributes for an event type, grouped by attribute type. Note that
          only attributes that contain values within the provided time window
          will be returned. You can use this to explore your data. You can also
          use it in automation, for things like ensuring customer data is
          reporting correctly.
        </Trans>
      </p>
      <SampleQuery
