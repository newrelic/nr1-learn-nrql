    ...
    <div>
      ...
      <p>
        <Trans i18nKey="Contents.P2">
          ...
          clauses. You must <strong>SELECT</strong> some data and tell us where
          it is <strong>FROM</strong>.
        </Trans>
      </p>
      ...
      <p>
        <Trans i18nKey="Contents.P4">
          This returns a lot of results. Each has a timestamp and a collection
          of <em>attributes</em>. For now, we only want a single result, so we
          will limit results to a single record using <code>LIMIT 1</code>.
        </Trans>
      </p>
      ...
      <p>
        <Trans i18nKey="Contents.P6">
          We're now controlling the volume of results we get. But what if we
          don't want <em>all</em> the attributes? What if we would prefer to see
          only specific data points? Fortunately, like SQL, this can be done in
          only a few characters. We replace <code>*</code> with the name of the
      ...
      <p>
        <Trans i18nKey="Contents.P8">
          Fantastic start. You just wrote basic queries that can return either
          all attributes of an event, or specific attributes. You also learned
          how to control the number of results returned. Great work!
        </Trans>
      </p>
    </div>
  );
