...
`,
  ...
  Summary_L1: {
    ...
    Contents: {
      ...
      <1>属性(アトリビュート)の指定</1>
      <2>集約関数の利用: <1>average()</1>, <3>max()</3>,
          <6>min()</6>, <8>sum()</8>, <10>count()</10></2>
      <3><1>SINCE</1>と<3>UNTIL</3>による時間帯の指定</3>
      <4><1>TIMESERIES</1>による時系列データの折れ線グラフへのプロット</4>
      <5><1>WHERE</1>句によるフィルタリング</5>
      <6><1>FACET</1>による属性でのグルーピング</6>
      `,
      P5: `
