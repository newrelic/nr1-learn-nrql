const translate = {
  ...
  'Aggregate functions': {
    ...
    Contents: {
      P1: `
        レベル1では、<3>count()</3>, <5>average()</5>, <7>sum()</7>, <9>max()</9>, <11>min()</11> を使ってデータを意味のある方法で変換するための集計について簡単に紹介しました。
      ...
      P11: `
        ...
        過去24時間の<1>percentile(duration, 98)</1>をNRDBに求めることができます。
      `,
      H1: `
        本レッスンのまとめ
      `,
  ...
  'Wildcard filters': {
    ...
    Contents: {
      ...
      `,
      P3: `
        これとは逆に、<2>NOT LIKE</2>を使用するようにクエリを変更すると、名前に "Web"という単語を含まないトランザクションの数が得られます。
        `,
      P4: `
